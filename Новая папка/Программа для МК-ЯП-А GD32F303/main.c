...
unsigned char Query[4],QueryCounter=0,SendBuf[2][14],SendBufWrite=0,SendBufRead=1,Timer2Flag=0;
unsigned short U_BI=0, U_NI=0, U_PWR=0, NPState=0, BPState=0;
...
void WriteFlash(unsigned char data)
{
	...
	FMC_KEY0=0xCDEF89AB;
	while ((FMC_STAT0&1)!=0) ;
	FMC_CTL0|=1<<1;													//Page Erase
	FMC_ADDR0=0x803F800;                                            //Последняя страница main flash
	FMC_CTL0|=1<<6;													//Start
	while ((FMC_STAT0&1)!=0) ;
	...
	FMC_KEY0=0xCDEF89AB;
	while ((FMC_STAT0&1)!=0) ;
	FMC_CTL0|=1;													//Main flash program
	*((unsigned char*)(0x0803f800))=data;
	while ((FMC_STAT0&1)!=0) ;
	__enable_irq();
}
...
int main()
{
	...
	RCU_APB1EN|=1<<1;										//Настраиваем Timer2 для сброса сигнала выбора источника питания через 8мс
	TIMER_PSC(TIMER2)=39;
	TIMER_CNT(TIMER2)=0;
	TIMER_CAR(TIMER2)=23999;                  		        //12мс
	TIMER_DMAINTEN(TIMER2)=1;
	NVIC_SetPriority(TIMER2_IRQn, 0); 
	NVIC_EnableIRQ(TIMER2_IRQn);

	//Чтение FLASH
	state=*((unsigned char*)(0x0803f800));
	state=state&2;											//Обнуляем для старта с НИ
	ChangeState(state);
	...
	while (1)
	{ 
		if ((command[0]>=50)&&((state&1)!=0)&&(Timer2Flag==0))	
		{
			state&=~1;
			ChangeState(state);										//Команда НИ
		}
		if ((command[1]>=50)&&((state&1)==0)&&(Timer2Flag==0))	
		{
			state|=1;
		...
		if (Flag_100Hz==1)
		{
			...
			U_PWR=(ADC_RDATA(ADC0)>>2)&0x3FF;

			ADC_RSQ2(ADC0)=1;
			for (i=0;i<100;i++);
			ADC_CTL1(ADC0)|=1<<22;															
			while ((ADC_STAT(ADC0)&2)==0) ;
			U_BI=(ADC_RDATA(ADC0)>>2)&0x3FF;

			ADC_RSQ2(ADC0)=2;
			for (i=0;i<100;i++);
			ADC_CTL1(ADC0)|=1<<22;															
			while ((ADC_STAT(ADC0)&2)==0) ;
			U_NI=(ADC_RDATA(ADC0)>>2)&0x3FF;

			SendBuf[SendBufWrite][0]=U_PWR;
			SendBuf[SendBufWrite][1]=U_PWR>>8;
			SendBuf[SendBufWrite][2]=U_NI;
			SendBuf[SendBufWrite][3]=U_NI>>8;
			SendBuf[SendBufWrite][4]=U_BI;
			SendBuf[SendBufWrite][5]=U_BI>>8;
		}
	}
...
{      
	...
	TIMER_INTF(TIMER2)&=~1;
	TIMER_CTL0(TIMER2)=0;
	if ((state&1)!=0) GPIO_BC(GPIOC)=1<<6;                                   		//GP_SEL-0
		else GPIO_BC(GPIOC)=1<<7;                                    				//BAT_SEL-0
	Timer2Flag=0;
}
	...
	if ((USART_STAT0(USART0)&0x20)!=0)
	{
		...
		if (QueryCounter==4) 
		{
			if (Crc8(Query,3)==Query[3]) 
			{
				...
				SendBuf[SendBufWrite][6]=NPState;
				SendBuf[SendBufWrite][7]=NPState>>8;
				SendBuf[SendBufWrite][8]=BPState;
				SendBuf[SendBufWrite][9]=BPState>>8;
