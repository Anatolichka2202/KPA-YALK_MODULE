...
Статус: production-инвариант интегрированного `master`.

## Цель

`master` является единым production-источником истины для стендового backend и принятого операторского UI: ScenarioEngine, процедуры, оборудование, протоколы, безопасные состояния, сценарии и presentation layer поставляются вместе. UI не должен менять backend-методику ради формы представления данных.
...
Текущий `master` публикует во время поканального чтения диагностический `BACKGROUND`-кадр: backend агрегирует `mean/min/max` одновременно по всем словам принятого кадра и после штатной калибровки отдаёт его как `RunEvent`. Принятый UI использует этот штатный контракт для общей плоскости ЯЛК и ЯТП. Параллельный reader того же UDP-потока только ради UI не добавляется.
...
Production-разработка УБСИ сведена в `master`.

Параллельно сохранены только два направления универсальной станции:

- `architecture/platform-delivery-ui` — архитектурный донор границ station/delivery/product;
- `universal_miltechstation` — исследовательская ветка общей станции и composition/runtime.

Они не являются кандидатами на wholesale merge в текущий КТМА/УБСИ. Решения из них переносятся в будущую универсальную станцию отдельными проверяемыми срезами.
