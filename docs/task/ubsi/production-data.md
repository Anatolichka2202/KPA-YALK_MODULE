...
# Целевое правило выборочной перепроверки

После `FullUbsi` оператор может запустить отдельный пакет `Yalk`, `Ytp`,
`Yvp` или `PowerConsumption` для того же зарегистрированного изделия.
Новый запуск не изменяет и не удаляет измерения предыдущего `FullUbsi`:
оба run остаются в истории вместе со снимками состава. Текущий статус
затронутого узла должен определяться последней принятой проверкой этого
узла; финальный ТУ выполняется отдельным запуском.

Это согласованное требование к production aggregation, **не утверждение о
готовности текущего кода**. До реализации и теста этого правила статус
изделия нельзя вычислять из одного последнего run без учёта его пакета.
