...
struct TestPage::Impl
{
    ...
    QWidget* buildYalk()
    {
        ...
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Поток ЯЛК"),yalkStream),1);m->addWidget(metricCard(QStringLiteral("Sequence"),yalkSequence),1);wl->addLayout(m);auto*p=panel();auto*pl=new QVBoxLayout(p);pl->addWidget(sectionLabel(QStringLiteral("Инициализация тракта ЯЛК")));pl->addWidget(subtitleLabel(QStringLiteral("Подготовка переключения и ожидание свежего потока ЯЛК-адаптера.")));pl->addStretch();wl->addWidget(p,1);yalkStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Ноль"),yalkCalZero),1);m->addWidget(metricCard(QStringLiteral("Полная шкала"),yalkCalFull),1);wl->addLayout(m);yalkCalTrend=new TrendPlot;yalkCalTrend->configure(QStringLiteral("Калибровочные данные"),QStringLiteral("В"),QStringLiteral("ноль"),QStringLiteral("6,2 В"));wl->addWidget(yalkCalTrend,1);yalkStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);wl->addWidget(sectionLabel(QStringLiteral("Исходное состояние 80 входов")));yalkInitial=new StateGrid;wl->addWidget(yalkInitial,1);yalkStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Адрес"),yalkChannel),1);m->addWidget(metricCard(QStringLiteral("Точка"),yalkPoint),1);m->addWidget(metricCard(QStringLiteral("U ИСД по В7"),yalkV7),1);wl->addLayout(m);yalkOverview=new ChannelOverview;yalkOverview->configure(80,QStringLiteral("В"));wl->addWidget(yalkOverview,1);yalkStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Точка"),yalkDiscretePoint),1);m->addWidget(metricCard(QStringLiteral("Expected"),yalkExpected),1);m->addWidget(metricCard(QStringLiteral("Адрес"),yalkDiscreteChannel),1);wl->addLayout(m);yalkDiscrete=new StateGrid;wl->addWidget(yalkDiscrete,1);yalkDiscreteSteps=new StepPlot;yalkDiscreteSteps->configure(QStringLiteral("Воздействие и ожидаемый признак"),QStringLiteral("В"),{0,0.9,2.5});wl->addWidget(yalkDiscreteSteps);yalkStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Канал"),overloadChannel),1);m->addWidget(metricCard(QStringLiteral("Воздействие"),overloadPolarity),1);m->addWidget(metricCard(QStringLiteral("Макс. |Δcode|"),overloadDelta),1);wl->addLayout(m);overloadOverview=new ChannelOverview;overloadOverview->configure(88,QStringLiteral("code"));wl->addWidget(overloadOverview,1);overloadSteps=new StepPlot;overloadSteps->configure(QStringLiteral("Воздействие перегрузки"),QStringLiteral("В"),{12,-12});wl->addWidget(overloadSteps);yalkStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("В7"),referenceV7),1);m->addWidget(metricCard(QStringLiteral("ЯЛК"),referenceYalk),1);m->addWidget(metricCard(QStringLiteral("|Δ|"),referenceDelta),1);wl->addLayout(m);referenceTrend=new TrendPlot;referenceTrend->configure(QStringLiteral("Эталон 6,2 В · В7 / ЯЛК"),QStringLiteral("В"),QStringLiteral("В7"),QStringLiteral("ЯЛК"));wl->addWidget(referenceTrend,1);yalkStack->addWidget(w);}
        l->addWidget(yalkStack,1);setYalkPhase(YalkPhase::Init);return page;
    }
    ...
    QWidget* buildYtp()
    {
        ...
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Поток ЯТП"),ytpStream),1);m->addWidget(metricCard(QStringLiteral("Endpoint"),ytpEndpoint),1);wl->addLayout(m);auto*p=panel();auto*pl=new QVBoxLayout(p);pl->addWidget(sectionLabel(QStringLiteral("Подготовка потока ЯТП")));pl->addWidget(subtitleLabel(QStringLiteral("Переключение тракта и ожидание данных.")));pl->addStretch();wl->addWidget(p,1);ytpStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Калибровочный ноль"),ytpCalZero),1);m->addWidget(metricCard(QStringLiteral("Калибровочная шкала"),ytpCalFull),1);wl->addLayout(m);ytpCalTrend=new TrendPlot;ytpCalTrend->configure(QStringLiteral("Калибровочные данные ЯТП"),QStringLiteral("Ом"),QStringLiteral("ноль"),QStringLiteral("шкала"));wl->addWidget(ytpCalTrend,1);ytpStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Канал"),ytpChannel),1);m->addWidget(metricCard(QStringLiteral("Р4831"),ytpReference),1);m->addWidget(metricCard(QStringLiteral("ЯТП"),ytpMeasured),1);wl->addLayout(m);ytpOverview=new ChannelOverview;ytpOverview->configure(30,QStringLiteral("Ом"));wl->addWidget(ytpOverview,1);ytpResistanceSteps=new StepPlot;ytpResistanceSteps->configure(QStringLiteral("Воздействие магазина сопротивления Р4831"),QStringLiteral("Ом"),{0,120,240});wl->addWidget(ytpResistanceSteps);ytpStack->addWidget(w);}
        l->addWidget(ytpStack,1);setYtpPhase(YtpPhase::Init);return page;
    }
    ...
    void resetWorkspace()
    {
        runInProgress=false;runClockTimer->stop();elapsed->setText(QStringLiteral("00:00:00"));progress->setValue(0);consumption->clear();powerTrend->clear();yalkCalTrend->clear();yalkOverview->clear();yalkDiscrete->clear();overloadOverview->clear();referenceTrend->clear();ytpCalTrend->clear();ytpOverview->clear();yvpTrend->clear();finishVerdict->setText(QStringLiteral("Формирование результата…"));finishVerdict->setStyleSheet(QStringLiteral("font-size:31px;font-weight:800;color:#d7a95b;"));reportButton->setEnabled(false);setTopStage(TopStage::Preparation);configureRouteVisibility();
    }
    ...
    {
        const QStringList names={QStringLiteral("Подготовка"),QStringLiteral("Питание / потребление"),QStringLiteral("ЯЛК-96"),QStringLiteral("ЯТП"),QStringLiteral("ЯВП-8"),QStringLiteral("Завершение")};for(int i=0;i<stageLabels.size();++i){auto*l=stageLabels[i];if(!l->isVisible())continue;if(i<static_cast<int>(topStage)){l->setObjectName(QStringLiteral("stageDone"));l->setText(QStringLiteral("✓  %1. %2\nзавершено").arg(i+1).arg(names[i]));}else if(i==static_cast<int>(topStage)){l->setObjectName(QStringLiteral("stageActive"));l->setText(QStringLiteral("▶  %1. %2\nтекущий этап").arg(i+1).arg(names[i]));}else{l->setObjectName(QStringLiteral("stage"));l->setText(QStringLiteral("○  %1. %2").arg(i+1).arg(names[i]));}l->style()->unpolish(l);l->style()->polish(l);}
    }
