...
struct TestPage::Impl
{
    ...
    QWidget* buildYalk()
    {
        ...
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Поток ЯЛК"),yalkStream),1);m->addWidget(metricCard(QStringLiteral("Sequence"),yalkSequence),1);wl->addLayout(m);wl->addStretch();yalkStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Ноль"),yalkCalZero),1);m->addWidget(metricCard(QStringLiteral("Полная шкала"),yalkCalFull),1);wl->addLayout(m);yalkCalTrend=new TrendPlot;yalkCalTrend->configure(QStringLiteral("Калибровка"),QStringLiteral("В"),QStringLiteral("0"),QStringLiteral("6,2"));wl->addWidget(yalkCalTrend,1);yalkStack->addWidget(w);}
        ...
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Текущий адрес"),yalkChannel),1);m->addWidget(metricCard(QStringLiteral("Точка"),yalkPoint),1);m->addWidget(metricCard(QStringLiteral("U ИСД по В7"),yalkV7),1);wl->addLayout(m);yalkOverview=new ChannelOverview;yalkOverview->setObjectName(QStringLiteral("yalkChannelHistogram"));yalkOverview->configure(80,QStringLiteral("В"));wl->addWidget(yalkOverview,1);yalkStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Точка воздействия"),yalkDiscretePoint),1);m->addWidget(metricCard(QStringLiteral("Ожидаемый признак"),yalkExpected),1);m->addWidget(metricCard(QStringLiteral("Текущий адрес"),yalkDiscreteChannel),1);wl->addLayout(m);yalkContacts=new ContactThresholdOverview;wl->addWidget(yalkContacts,1);yalkStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("Нагружаемый канал"),overloadChannel),1);m->addWidget(metricCard(QStringLiteral("Полярность"),overloadPolarity),1);m->addWidget(metricCard(QStringLiteral("Макс. |Δcode| остальных"),overloadDelta),1);m->addWidget(metricCard(QStringLiteral("Ход воздействия"),overloadProgress),1);wl->addLayout(m);overloadOverview=new OverloadOverview;wl->addWidget(overloadOverview,1);yalkStack->addWidget(w);}
        {auto*w=new QWidget;auto*wl=new QVBoxLayout(w);wl->setContentsMargins(0,0,0,0);auto*m=new QHBoxLayout;m->addWidget(metricCard(QStringLiteral("В7"),referenceV7),1);m->addWidget(metricCard(QStringLiteral("ЯЛК"),referenceYalk),1);m->addWidget(metricCard(QStringLiteral("|Δ|"),referenceDelta),1);wl->addLayout(m);referenceTrend=new TrendPlot;referenceTrend->configure(QStringLiteral("Эталон 6,2 В"),QStringLiteral("В"),QStringLiteral("В7"),QStringLiteral("ЯЛК"));wl->addWidget(referenceTrend,1);yalkStack->addWidget(w);}
    ...
    void resetWorkspace()
    {
        runInProgress=false;runClockTimer->stop();elapsed->setText(QStringLiteral("00:00:00"));progress->setValue(0);consumption->clear();powerTrend->clear();yalkCalTrend->clear();yalkOverview->clear();yalkContacts->clear();overloadOverview->clear();referenceTrend->clear();ytpCalTrend->clear();ytpOverview->clear();yvpTrend->clear();finishVerdict->setText(QStringLiteral("Формирование результата…"));finishVerdict->setStyleSheet(QStringLiteral("font-size:31px;font-weight:800;color:#d7a95b;"));reportButton->setEnabled(false);setTopStage(TopStage::Preparation);configureRouteVisibility();
    }
