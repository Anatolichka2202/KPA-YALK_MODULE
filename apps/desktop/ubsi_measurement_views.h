    ...
    void paintEvent(QPaintEvent*) override
    {
        ...
        for (int i = 0; i < values.size(); ++i) {
            ...
            if (std::isfinite(value.value)) {
                ++rendered;
                const QColor color = value.warning ? palette::amber
                    : value.state == VerificationState::NeNorma
...
protected:
    ...
    void paintEvent(QPaintEvent*) override
    {
        ...
        double baseLo = std::numeric_limits<double>::infinity();
        double baseHi = -std::numeric_limits<double>::infinity();
        for (const auto& channel : frame_.channels) {
            if (std::isfinite(channel.baselineCode)) {
                baseLo = std::min(baseLo, channel.baselineCode);
                baseHi = std::max(baseHi, channel.baselineCode);
            }
            if (std::isfinite(channel.currentCode)) {
                baseLo = std::min(baseLo, channel.currentCode);
                baseHi = std::max(baseHi, channel.currentCode);
            }
        }
        if (!std::isfinite(baseLo) || !std::isfinite(baseHi)) {
            p.setPen(palette::dim);
            p.drawText(area, Qt::AlignCenter, QStringLiteral("ожидание baseline / current"));
            return;
        }
        const double pad = std::max(1.0, (baseHi - baseLo) * .16);
        baseLo -= pad;
        baseHi += pad;
        if (!(baseHi > baseLo)) baseHi = baseLo + 1.0;
        const double center = (baseLo + baseHi) / 2.0;
        const double viewWidth = (baseHi - baseLo) / std::max(1.0, zoomScale_);
        const double lo = center - viewWidth / 2.0;
        const double hi = center + viewWidth / 2.0;
        setProperty("viewMinimum", lo);
        setProperty("viewMaximum", hi);
        ...
        for (int tick = 0; tick <= 4; ++tick) {
            const double yy = area.top() + tick * area.height() / 4.0;
            p.setPen(QPen(palette::lineSoft, 1, Qt::DashLine));
        ...
        double maxDelta = 0.0;
        for (int i = 0; i < frame_.channels.size() && i < 88; ++i) {
            const auto& channel = frame_.channels[i];
            const double x = area.left() + i * cell;
            ...
            if (std::isfinite(channel.baselineCode)) {
                const double top = yFor(channel.baselineCode);
                p.fillRect(QRectF(x + cell * .08, top,
                                  std::max(2.0, cell * .84), area.bottom() - top),
                           QColor(142, 166, 183, 70));
            }
            if (std::isfinite(channel.currentCode)) {
                maxDelta = std::max(maxDelta, std::abs(channel.deltaCode));
                const double top = yFor(channel.currentCode);
                const QColor color = channel.verification == VerificationState::NeNorma
                    || channel.verification == VerificationState::Error ? palette::red : palette::cyan;
                p.fillRect(QRectF(x + cell * .28, top,
                                  std::max(2.0, cell * .44), area.bottom() - top), color);
        ...
        p.drawText(QRectF(area.left(), 8, area.width(), 20), Qt::AlignLeft,
                   QStringLiteral("baseline + current · критерий |current − baseline| ≤ %1 кода")
