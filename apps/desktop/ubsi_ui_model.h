...
public:
    ...
    void reset()
    {
        ...
        for (int channel = 1; channel <= 88; ++channel) {
            OverloadChannel c; c.physicalAddress = channel; yalkOverload.channels.push_back(c);
        }
...
private:
    static int findAddress(const QVector<AnalogChannel>& channels, int address)
    {
        for (int i = 0; i < channels.size(); ++i) if (channels[i].physicalAddress == address) return i;
        return -1;
    }
    static int findContact(const QVector<ContactChannel>& channels, int address)
    {
        for (int i = 0; i < channels.size(); ++i) if (channels[i].physicalAddress == address) return i;
        return -1;
    }
    ...
    void applyYalkAnalog(const orbita::stand::RunEvent& event)
    {
        ...
        channel.verification = verificationFromVerdict(event.verdict);
        const double lo = eventDouble(event, "lower_limit_v");
        const double hi = eventDouble(event, "upper_limit_v");
        channel.warning = channel.verification == VerificationState::Norma
    ...
    {
        const int address = eventInt(event, "ulk_address");
        const int index = findContact(yalkContact.channels, address);
        if (index < 0) return;
        auto samples = csvNumbers(eventValue(event, "value_samples"));
        double minimum = eventDouble(event, "yalk_v");
        double maximum = minimum;
    ...
    void applyYalkOverload(const orbita::stand::RunEvent& event)
    {
        ...
        if (QString::fromStdString(event.stage) == QStringLiteral("MEASUREMENT")) {
            ...
            if (observed >= 1 && observed <= yalkOverload.channels.size()) {
                auto& channel = yalkOverload.channels[observed - 1];
