...
int main(int argc,char** argv)
{
    ...
    for(int channel=1;channel<=8;++channel){orbita::stand::RunEvent e;e.nodeId="yvp_measurement";e.stage="YVP_V7_POINT";e.verdict=orbita::stand::RunVerdict::NotRun;e.data={{"yvp_channel",std::to_string(channel)},{"gain_mv_per_pc","1"},{"set_frequency_hz","500"},{"rigol_input_vpp","2"},{"v7_output_vrms",std::to_string(.141+channel*.001)},{"calculated_gain_mv_per_pc",std::to_string(.98+channel*.004)},{"acceptance","evaluated_after_gain_sweep"}};page.setRunEvent(e);}QApplication::processEvents();require(yvp->property("yvpRenderedChannelCount").toInt()==8,"YVP plane must show eight channels");saveScene(page,requested,QStringLiteral("YVP"),screenshot);

    orbita::stand::ScenarioRunResult result;result.runId="prototype-acceptance";result.verdict=orbita::stand::RunVerdict::Ok;auto addStep=[&](const char* id,orbita::stand::RunVerdict verdict){orbita::stand::StepRunResult s;s.nodeId=id;s.verdict=verdict;result.steps.push_back(std::move(s));};addStep("supply_status",orbita::stand::RunVerdict::Ok);addStep("yalk_initial",orbita::stand::RunVerdict::Ok);addStep("ytp_channels",orbita::stand::RunVerdict::Ok);addStep("yvp_measurement",orbita::stand::RunVerdict::Ok);page.setRunResult(result);saveScene(page,requested,QStringLiteral("FINISH"),screenshot);
