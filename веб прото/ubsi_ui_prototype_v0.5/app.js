...
(() => {
  ...
  // Current production stages from registrar::Stage.
  // Human-readable delivery labels are not invented in the prototype: until
  // delivery configuration provides them, the canonical Stage code is shown.
  const productionStages = [
    { code:'Primary' },
    { code:'ClimateNormal' },
    { code:'ClimateMinus' },
    { code:'ClimatePlus' },
    { code:'PottingClimateNormal' },
    { code:'PottingClimatePlus' },
    { code:'PottingClimateMinus' },
  ];
  const stages = productionStages.map(x=>x.code);
        ...
        <div class="home">
          <div class="home-card">
            ...
            <div class="phase-note">
              Прототип v0.5: Production, ТУ и Администрирование кликабельны. Production использует 7 актуальных registrar-stage. PROTO-контролы существуют только для демонстрации состояний.
            </div>
          </div>
        </div>
      </div>`;
  ...
  function renderSession(){
    app.innerHTML = `
      <div class="app-shell page">
        ${topbar('ПРОИЗВОДСТВО','производственная сессия')}
        <div class="session">
          <div class="session-controls">
            <div class="field">
              <label>Оператор</label>
              <div style="display:flex;gap:6px">
                <select class="select" id="operator-select">
                  ${operators.map(o=>`<option ${o===state.operator?'selected':''}>${escapeHtml(o)}</option>`).join('')}
                </select>
                <button class="btn btn-small" id="add-operator" title="Добавить оператора">＋</button>
              </div>
            </div>
            <div class="field">
              <label>Производственный этап</label>
              <select class="select" id="stage-select">
                ${stages.map(s=>`<option value="${escapeHtml(s)}" ${s===state.productionStage?'selected':''}>${escapeHtml(s)}</option>`).join('')}
              </select>
              <div class="field-help">7 этапов текущей production-модели</div>
            </div>
            <div class="field">
              <label>Объём проверки</label>
              <div class="scope-row">
                ${scopes.map(s=>`<button class="scope-chip ${s===state.scope?'active':''}" data-scope="${escapeHtml(s)}">${escapeHtml(s)}</button>`).join('')}
              </div>
            </div>
          </div>
          ...
          <div class="session-grid">
            <section class="list-panel">
              <div class="panel-title" style="display:flex;align-items:center;gap:8px">
                <span>Зарегистрированные УБСИ</span><span style="flex:1"></span>
                <button class="btn btn-small" id="add-selected-queue">Добавить в очередь →</button>
              </div>
              <div class="search-wrap"><input id="registry-search" class="input" placeholder="Поиск по SN..." /></div>
              <div class="registry-list" id="registry-list"></div>
            </section>

            <section class="list-panel queue-panel">
              <div class="panel-title">Очередь текущей сессии</div>
              <div class="queue-list" id="queue-list"></div>
            </section>
          </div>

          <div class="session-footer">
            <button class="btn btn-ghost" id="back-home">← Главная</button>
            <div class="summary" id="queue-summary"></div>
            <div class="spacer"></div>
            <button class="btn btn-primary" id="start-prep" ${state.queue.length?'':'disabled'}>Перейти к подготовке</button>
          </div>
        </div>
      </div>`;
    ...
    document.getElementById('add-selected-queue').onclick=()=>{
      if(state.selectedRegistry && !state.queue.includes(state.selectedRegistry)){
        state.queue.push(state.selectedRegistry);paintQueue();
      }
    };
    document.getElementById('stage-select').onchange=e=>state.productionStage=e.target.value;
    document.querySelectorAll('[data-scope]').forEach(btn=>{
      btn.onclick=()=>{ state.scope=btn.dataset.scope; renderSession(); };
    });
    const search = document.getElementById('registry-search');
    search.oninput=()=>paintRegistry(search.value);
    document.getElementById('start-prep').onclick=()=>{
      state.activeQueueIndex=0;
      state.prepChecked=false; state.prepBusy=false; state.prepOk=false;
      setScreen('prep');
    };
    paintRegistry('');
    paintQueue();
  ...
  function paintRegistry(q){
    ...
    root.innerHTML=items.map(x=>`
      <div class="registry-row ${state.selectedRegistry===x.serial?'selected':''}" data-serial="${x.serial}">
        <div class="serial">SN ${x.serial}</div>
        <div class="meta">${x.type}</div>
        <div class="meta">${x.date}</div>
      </div>`).join('');
    root.querySelectorAll('.registry-row').forEach(row=>{
      row.onclick=()=>{ state.selectedRegistry=row.dataset.serial; paintRegistry(q); };
      row.ondblclick=()=>{
        if(!state.queue.includes(row.dataset.serial)) state.queue.push(row.dataset.serial);
        paintQueue(); paintRegistry(q);
      };
    });
  }
  function paintQueue(){
    ...
    if(!state.queue.length){
      root.innerHTML=`<div style="padding:18px;color:var(--muted)">Очередь пуста. Добавьте УБСИ двойным щелчком из списка слева.</div>`;
    } else {
      root.innerHTML=state.queue.map((serial,i)=>`
        <div class="queue-row">
          <div class="queue-index">${i+1}</div>
          <div><strong>SN ${serial}</strong></div>
          <div class="queue-status">${i<state.activeQueueIndex?'ЗАВЕРШЕНО':i===state.activeQueueIndex&&state.screen!=='session'?'ТЕКУЩЕЕ':'ОЖИДАЕТ'}</div>
          <div class="queue-actions">
            <button class="btn btn-small" data-up="${i}" ${i===0?'disabled':''}>↑</button>
            <button class="btn btn-small" data-down="${i}" ${i===state.queue.length-1?'disabled':''}>↓</button>
            <button class="btn btn-small" data-remove="${i}">×</button>
          </div>
        </div>`).join('');
      root.querySelectorAll('[data-up]').forEach(b=>b.onclick=()=>moveQueue(+b.dataset.up,-1));
      root.querySelectorAll('[data-down]').forEach(b=>b.onclick=()=>moveQueue(+b.dataset.down,1));
      root.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{
        state.queue.splice(+b.dataset.remove,1); paintQueue();
      });
    }
    const s=document.getElementById('queue-summary');
    if(s) s.textContent=`В очереди: ${state.queue.length} · ${state.scope} · ${state.productionStage}`;
  }
  function moveQueue(i,dir){
    const j=i+dir;if(j<0||j>=state.queue.length)return;
  ...
  function showAddOperatorModal(){
    modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal">
      ...
      <div class="field" style="margin-top:12px"><label>ФИО</label><input class="input" id="new-operator-name" placeholder="Фамилия Имя Отчество" /></div>
    ...
    document.getElementById('new-operator-ok').onclick=()=>{
      ...
      const short=parts.length>=3 ? `${parts[0]} ${parts[1][0]}.${parts[2][0]}.` : raw;
      if(!operators.includes(short)) operators.push(short);
      state.operator=short;
      modalRoot.innerHTML='';
      renderSession();
    };
  }
      ...
      <div class="app-shell page">
        ...
        <div class="prep">
          <div class="prep-head">
            ...
            <p>Этап ${escapeHtml(state.productionStage)} · ${escapeHtml(state.scope)}. Проверяется только оборудование, требуемое выбранным сценарием.</p>
          </div>
  ...
  function makeRun(){
    ...
    const run = {
      ...
      stopped:false, standError:false, productFail:false,
      plan:[
      ...
      yalk:{
        ...
        analogSnapshots:{}, viewingPoint:null,
      ...
      ytp:{
        pointIndex:0,channel:0,points:[0,120,240],frame:makeYtpFrame(0),waiting:false,
        phase:'settle',phaseElapsed:0
      },
      yvp:{
  ...
  function makeDiscreteFrame(point){
    return Array.from({length:80},(_,i)=>({
      analog:point + Math.sin(i*.23)*.035 + (Math.random()-.5)*.025,
      logic: point>=2.5 ? 1 : 0,
      state:'pending'
    }));
  }
  ...
  function tickRun(r,dt){
    if(r.stopped || r.standError || r.ytp.waiting) return;
    r.displayElapsed += dt;
    r.stageElapsed += dt;

    // total current always whole UBSI
    if(r.currentFresh){
    ...
    if(r.stageElapsed>=1.15){
      ...
      if(y.analogChannel>=80){
        y.analogSnapshots[nominal]={
          nominal,actual:ref,channels:y.analogFrame.slice(),completedAt:r.displayElapsed
        };
  ...
  function tickDiscrete(r,dt){
    ...
    y.discreteFrame=y.discreteFrame.map((c,i)=>({
      ...c,
      analog:c.analog+(point-c.analog)*.22+(Math.random()-.5)*.013,
      logic: point>=2.5?1:0
    }));
    if(r.stageElapsed>=1.15){
  ...
  function tickOverload(r,dt){
    ...
    if(y.overloadPhase==='impact'){
      ...
      y.overloadCurrent=y.overloadBaseline.map((v,i)=>{
        if(i===y.overloadChannel) return v; // stressed channel is excluded from criterion
        const coupling=(Math.sin((i+1)*(y.overloadChannel+3)*.021)+Math.cos(i*.19))*.006;
        const polaritySign=y.overloadPolarity>0?1:-1;
        const transient=polaritySign*coupling*(.25+.75*t);
        const random=(Math.random()-.5)*.003;
        return v+transient+random;
      });
      if(r.failChannel!=null && r.failChannel!==y.overloadChannel){
        y.overloadCurrent[r.failChannel]=y.overloadBaseline[r.failChannel]+.075*(y.overloadPolarity>0?1:-1);
      }
      y.overloadHold=y.overloadPhaseElapsed;

      const deltas=y.overloadCurrent.map((v,i)=>i===y.overloadChannel?0:Math.round((v-y.overloadBaseline[i])/.02));
      y.overloadMaxDelta=Math.max(...deltas.map(Math.abs));
  ...
  function tickYtp(r,dt){
    ...
    if(y.phase==='read' && y.phaseElapsed>=0.25){
      y.channel++;
      y.phaseElapsed=0;
      if(y.channel>=30){
        y.channel=0;
  ...
  function setStage(r,stage){
    ...
    if(stage==='ytp'){r.ytp.phase='settle';r.ytp.phaseElapsed=0}
  ...
  function finishRun(){
    ...
    if(!r.finishedBlocks.yvp)r.finishedBlocks.yvp='НЕПОЛНАЯ';
  ...
  function renderRoute(r){
    const activeBlock=stageRouteState(r,'power').activeBlock;
    return route.map(block=>{
      const rs=stageRouteState(r,block.id);
      const canView=rs.done && ['power','yalk','ytp','yvp'].includes(block.id);
      const expanded = rs.active || r.routeExpanded===block.id;
      let sub='';
      if(block.id==='yalk' && expanded){
        const activeSub = r.stage==='analog'?'analog':r.stage==='discrete'?'discrete':r.stage==='overload'?'overload':'';
        const reached = {
          stream:true,cal:true,initial:true,
          analog: ['analog','discrete','overload','ytp','yvp'].includes(r.stage) || !!r.finishedBlocks.yalk,
          discrete: ['discrete','overload','ytp','yvp'].includes(r.stage) || !!r.finishedBlocks.yalk,
          overload: ['overload','ytp','yvp'].includes(r.stage) || !!r.finishedBlocks.yalk,
          ref: !!r.finishedBlocks.yalk || ['ytp','yvp'].includes(r.stage),
          'yalk-clean': !!r.finishedBlocks.yalk || ['ytp','yvp'].includes(r.stage)
        };
        sub=`<div class="substeps">${block.sub.map(([id,label])=>{
          const cls=id===activeSub?'active':(reached[id]&&id!==activeSub?'done':'');
          const viewStage=id==='analog'?'analog':id==='discrete'?'discrete':id==='overload'?'overload':null;
          return `<div class="substep ${cls}" ${viewStage&&reached[id]&&id!==activeSub?`data-route-view="${viewStage}"`:''}>
            <span>${cls==='done'?'✓':id===activeSub?'●':'○'}</span>${label}
          </div>`;
        }).join('')}</div>`;
      }
      if(block.id==='ytp' && expanded){
        sub=`<div class="substeps">
          <div class="substep done">✓ Инициализация</div>
          <div class="substep done">✓ Калибровка</div>
          <div class="substep ${rs.active?'active':'done'}" ${!rs.active?'data-route-view="ytp"':''}>${rs.active?'●':'✓'} 30 каналов</div>
          <div class="substep ${rs.done?'done':''}">${rs.done?'✓':'○'} Завершение ЯТП</div>
        </div>`;
      }
      if(block.id==='yvp' && expanded){
        sub=`<div class="substeps">
          <div class="substep done">✓ Инициализация</div>
          <div class="substep ${rs.active?'active':'done'}" ${!rs.active?'data-route-view="yvp"':''}>${rs.active?'●':'✓'} 8 каналов</div>
          <div class="substep ${rs.done?'done':''}">${rs.done?'✓':'○'} Завершение ЯВП</div>
        </div>`;
      }
      let mainView='';
      if(block.id==='power'&&rs.done)mainView='data-route-view="power"';
      if(block.id==='ytp'&&rs.done)mainView='data-route-view="ytp"';
      if(block.id==='yvp'&&rs.done)mainView='data-route-view="yvp"';
      if(block.id==='yalk'&&rs.done)mainView='data-route-expand="yalk"';
      return `<div class="route-block">
        <div class="route-main ${rs.active?'active':''} ${rs.done?'done':''}" ${mainView}>
          <span class="route-icon">${rs.done?'✓':rs.active?'●':'○'}</span>
    ...
    app.innerHTML=`
      <div class="app-shell runtime">
        <div class="run-header">
          ...
          <div class="branch">Производство · ${state.productionStage} · ${state.scope}</div>
          <div class="spacer"></div>
  ...
  function stageMetricsHtml(r,stage){
    ...
    if(stage==='power'){
      ...
      m=[
        ['Канал',`${ch+1} / 8`],
        ['Коэффициент',`${gain} мВ/пКл`],
        ['Частота',`${freq} Гц`],
        ['Точка матрицы',`${pointIndex} / ${total}`],
        ['Измеренное',fmt(v.measured,3)],
        ['Backend','MAPPING НЕ ПОДТВЕРЖДЁН','wait']
      ];
    }
  ...
  function analogView(r){
    ...
    return `<section class="stage-view">
      ...
      ${barChartHtml(values,{min:nominal===0?-.12:nominal-.15,max:nominal===0?.16:nominal+.16,reference:actual,tolerance:.08,stimulated:channel,pinned:y.pinned,kind:'analog'})}
    </section>`;
  }
    ...
    return `<section class="stage-view">
      ...
      <div class="digital-split">
        ...
        <div class="analog-card">
          ${barPlotOnly(y.discreteFrame.map(c=>c.analog),{min:point-.18,max:point+.18,reference:y.discreteActual??point,tolerance:.09,stimulated:y.discreteChannel,pinned:y.pinned,kind:'discrete'})}
        </div>
      </div>
    </section>`;
  }
  ...
  function overloadView(r){
    const y=r.yalk;
    const values=(y.overloadPhase==='safe'||y.overloadPhase==='snapshot') && y.overloadLastSnapshot
      ? y.overloadLastSnapshot : y.overloadCurrent;
    const min=Math.min(...y.overloadBaseline,...values)-.08;
    const max=Math.max(...y.overloadBaseline,...values)+.08;
    return `<section class="stage-view">
      <div class="stage-title-row"><h2>ЯЛК-96 — перегрузка ±12 В</h2>
        <div class="hint">baseline снят один раз до перегрузки · сначала +12 В все 88 каналов, затем −12 В все 88</div><div class="spacer"></div>
      </div>
      <div class="overload-method">
        <span><i class="legend-shadow"></i> тень — baseline до первой перегрузки</span>
        <span><i class="legend-current"></i> столбец — текущий / последний свежий snapshot</span>
        <span><i class="legend-blue"></i> синий — перегружаемый канал (из критерия исключён)</span>
        <span>между каналами: источник OFF → RESET → ЦАП OFF → 0,3 с</span>
      </div>
      ${overloadChartHtml(y.overloadBaseline,values,{min,max,stimulated:y.overloadPhase==='impact'?y.overloadChannel:-1,pinned:y.pinned})}
    </section>`;
  }
  ...
  function ytpView(r){
    ...
    return `<section class="stage-view">
      ...
      ${barChartHtml(y.frame,{min,max,reference:p,tolerance:2,stimulated:y.channel,pinned:y.pinned,kind:'ytp',unit:' Ом'})}
    </section>`;
  }

  function yvpView(r){
    return `<section class="stage-view">
      <div class="stage-title-row"><h2>ЯВП-8 — измерительная матрица</h2><span class="demo-badge">ЦЕЛЕВОЙ VIEW · CURRENT MASTER: mapping_confirmed=false</span><div class="spacer"></div></div>
      <div class="yvp-grid">
        ${r.yvp.frame.map((v,i)=>yvpCard(i,v,i===r.yvp.channel)).join('')}
      </div>
    </section>`;
  }
  function yvpCard(i,v,active){
    const vals=[v.actual,v.reference,v.measured,v.isd];
    const min=Math.min(...vals)-.08,max=Math.max(...vals)+.08;
    const pos=x=>clamp(6+((x-min)/(max-min))*86,6,92);
    return `<div class="yvp-card" style="${active?'border-color:var(--blue-2);box-shadow:inset 0 0 0 1px rgba(88,165,255,.3)':''}">
      <div class="yvp-head"><strong>Канал ${i+1}</strong><span>${active?'текущий':''}</span></div>
      <div class="yvp-overlay">
        <div class="yvp-line actual" style="top:${100-pos(v.actual)}%"></div>
        <div class="yvp-line reference" style="top:${100-pos(v.reference)}%"></div>
        <div class="yvp-line measured" style="top:${100-pos(v.measured)}%"></div>
        <div class="yvp-line isd" style="top:${100-pos(v.isd)}%"></div>
      </div>
      <div class="yvp-legend">
        <div>Факт ${fmt(v.actual,3)}</div><div>Эталон ${fmt(v.reference,3)}</div>
        <div>Измерено ${fmt(v.measured,3)}</div><div>ИСД ${fmt(v.isd,3)}</div>
      </div>
    </div>`;
  }
  ...
  function overloadChartHtml(base,current,opt){
    const [rmin,rmax]=resolvedRange('overload',opt.min,opt.max);
    const range=rmax-rmin||1;
    const yPct=v=>clamp((v-rmin)/range*100,0,100);
    return `<div class="chart-card">
      <div class="chart-head"><strong>88 каналов · baseline + current</strong><span class="meta">baseline — тень, current — основной столбец</span><div class="spacer"></div></div>
      <div class="plot">
        <div class="y-grid">
          ${Array.from({length:5},(_,i)=>{const val=rmin+(rmax-rmin)*(4-i)/4;return `<div class="y-grid-line" style="top:${i*25}%"><span class="y-label">${fmt(val,3)}</span></div>`}).join('')}
        </div>
        <div class="bar-row overload-row">
          ${current.map((v,i)=>{
            const delta=Math.round((v-base[i])/.02),failed=Math.abs(delta)>2;
            return `<div class="bar-slot" data-overload="${i}" data-delta="${delta}">
              <div class="baseline" style="height:${Math.max(1,yPct(base[i]))}%"></div>
              <div class="bar current ${failed?'failed':''} ${i===opt.stimulated?'stimulated':''} ${i===opt.pinned?'pinned':''}" style="height:${Math.max(1,yPct(v))}%">
                <span class="bar-label">${fmt(v,3)}</span>
              </div>
              <span class="x-label">${(i+1)%8===0||i===0?i+1:''}</span>
            </div>`;
          }).join('')}
        </div>
      </div>
  ...
  function telemetryHtml(r){
    ...
    return `
      <div class="tele-box">
        ...
        <div class="tele-sub">${r.etaMode==='waiting_operator'?'Осталось: ожидание оператора':`Осталось ≈ ${formatDuration(eta)}`}</div>
      </div>
      <div class="tele-box">
  ...
  function stageProgress(r){
    ...
    if(r.stage==='ytp'){
      const y=r.ytp;return {label:`ЯТП · ${y.points[Math.min(y.pointIndex,2)]} Ом`,text:`канал ${y.channel+1} / 30`,fraction:(y.pointIndex*30+y.channel)/90,sub:y.waiting?'ожидание оператора':y.phase==='settle'?`стабилизация ${fmt(y.phaseElapsed,1)} / 1,5 с`:`считывание · точка ${Math.min(y.pointIndex+1,3)} / 3`};
    }
    if(r.stage==='yvp'){
      const y=r.yvp,total=8*y.gains.length*y.frequencies.length;
      const done=y.channel*y.gains.length*y.frequencies.length+y.gainIndex*y.frequencies.length+y.frequencyIndex;
      return {label:'ЯВП-8 · целевая матрица V7/ИСД',text:`${done+1} / ${total}`,fraction:done/total,sub:`канал ${y.channel+1}/8 · K=${y.gains[y.gainIndex]} мВ/пКл · f=${y.frequencies[y.frequencyIndex]} Гц`};
    }
  ...
  function showR4831Modal(value){
    ...
    document.getElementById('confirm-r4831').onclick=()=>{
      modalRoot.innerHTML='';
      const activeRun = state.screen==='tu-runtime' ? state.tu.run : state.run;
      if(!activeRun) return;
      activeRun.ytp.waiting=false;
      activeRun.etaMode='planned';
      activeRun.ytp.frame=makeYtpFrame(value);
      activeRun.ytp.phase='settle';
      activeRun.ytp.phaseElapsed=0;
      if(state.screen==='tu-runtime') paintTuRuntimeLive();
      else paintRuntimeLive();
    };
  }
  ...
  function renderFinish(){
    ...
    const incomplete=Object.values(r?.finishedBlocks||{}).includes('НЕПОЛНАЯ');
    const verdict=stand?'ОШИБКА СТЕНДА':stopped?'ОСТАНОВЛЕНО':bad?'НЕ НОРМА':incomplete?'НЕПОЛНАЯ':'НОРМА';
    ...
    app.innerHTML=`
      <div class="app-shell page">
        ${topbar('ПРОИЗВОДСТВО',`SN ${r.serial} · ${state.productionStage} · завершение`)}
        <div class="finish">
          <div class="finish-grid">
            <section class="result-card">
              ...
              <div style="margin-top:14px" class="field">
                ...
                <textarea class="comment" placeholder="Необязательный комментарий к этому прогону"></textarea>
              </div>
            ...
            <section class="result-card">
              ...
              <div class="detail-list">
                ...
                <div class="detail-item"><strong>Production stage</strong><span>${state.productionStage}</span></div>
                <div class="detail-item"><strong>Scope</strong><span>${state.scope}</span></div>
                ...
                <div class="detail-item"><strong>ЯВП</strong><span>8 каналов · normalized demo contract</span></div>
              </div>
            </section>
          </div>
        </div>
      </div>`;
    document.getElementById('back-session-finish').onclick=()=>{state.run=null;setScreen('session')};
    ...
    document.getElementById('next-item').onclick=()=>{
      if(state.activeQueueIndex<state.queue.length-1){
        state.activeQueueIndex++;state.prepChecked=false;state.prepOk=false;state.prepBusy=false;state.run=null;setScreen('prep');
      }
    };
  }
  ...
  function showProductionReport(r,blocks,verdict){
    modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal" style="width:min(760px,100%)">
      ...
      <div class="detail-list" style="margin-top:12px">
        ...
        <div class="detail-item"><strong>Production stage</strong><span>${escapeHtml(state.productionStage)}</span></div>
        <div class="detail-item"><strong>Scope</strong><span>${escapeHtml(state.scope)}</span></div>
  ...
  function renderProtoPanel(){
    ...
    wrap.querySelectorAll('[data-proto]').forEach(b=>b.onclick=()=>{
      ...
      if(state.screen==='runtime'){
        ...
        state.tu.reportMode='stand_error';state.protoOpen=false;setScreen('tu-operator');
        ...
        if(a==='tu-fail'){
          ...
          stopSimulation();state.tu.reportMode='stand_error';state.protoOpen=false;setScreen('tu-operator');return;
        }
  ...
  function renderTuEntry(){
    ...
    app.innerHTML=`
      <div class="app-shell page">
        ...
        <div class="tu-entry">
          <div>
            <h2 style="margin:0 0 6px">Выбор изделия</h2>
            <div style="color:var(--muted);font-size:13px">Выберите зарегистрированное УБСИ или введите SN вручную.</div>
          </div>
          <div class="tu-entry-grid">
            <section class="tu-choice">
              <h3>Зарегистрированное УБСИ</h3>
              <select class="select" id="tu-registry">
                ${registry.map(x=>`<option value="${x.serial}" ${t.serial===x.serial?'selected':''}>SN ${x.serial}</option>`).join('')}
              </select>
              <p>Используется запись из регистратора. Отдельная производственная сессия не создаётся.</p>
              <button class="btn ${t.serialMode==='registry'?'btn-primary':''}" id="tu-use-registry">Использовать выбранное</button>
            </section>
            <section class="tu-choice">
              <h3>SN вручную</h3>
              <input class="input" id="tu-manual" placeholder="Введите SN..." value="${escapeHtml(t.manualSerial)}" />
              <p>Для проверки изделия, которое не требуется заранее добавлять в production registry.</p>
              <button class="btn ${t.serialMode==='manual'?'btn-primary':''}" id="tu-use-manual">Использовать введённый SN</button>
            </section>
          </div>
          <div class="session-footer">
            <button class="btn btn-ghost" id="tu-home">← Главная</button>
            <div class="spacer"></div>
            <button class="btn btn-primary" id="tu-check-ready">Проверить стенд</button>
          </div>
        </div>
      </div>`;
    document.getElementById('tu-home').onclick=()=>setScreen('home');
    document.getElementById('tu-registry').onchange=e=>{t.serial=e.target.value;t.serialMode='registry'};
    document.getElementById('tu-manual').oninput=e=>t.manualSerial=e.target.value;
    document.getElementById('tu-use-registry').onclick=()=>{t.serialMode='registry';renderTuEntry()};
    document.getElementById('tu-use-manual').onclick=()=>{t.serialMode='manual';renderTuEntry()};
    document.getElementById('tu-check-ready').onclick=()=>{
      const serial=t.serialMode==='manual'?t.manualSerial.trim():t.serial;
      if(!serial){showInfoModal('SN не указан','Введите серийный номер изделия.');return}
      t.serial=serial;t.ready=false;
      setScreen('tu-ready');
      setTimeout(()=>{if(state.screen==='tu-ready'){t.ready=true;renderTuReady()}},800);
    };
  }
  ...
  function syncTuRequirement(r,beforeStage){
    // A compact prototype mapping: when the shared stage advances,
    // complete every TU row tied to the previous measurement stage and move to the first row of next stage.
    const idx=r.tuIndex;
    if(idx>=r.tuResults.length)return;
    const req=r.tuResults[idx];
    req.status='running';

    // Progress one normative row every few simulated seconds while same measurement view is live.
    req._elapsed=(req._elapsed||0)+0.38;
    if(req._elapsed>8.0){
      req.status=(r.tuProductFail && (idx===4||idx===7))?'ne_norma':'norma';
      r.tuIndex++;
      if(r.tuIndex>=r.tuResults.length){
        stopSimulation();
        state.tu.reportMode=r.tuResults.some(x=>x.status==='ne_norma')?'ne_norma':'norma';
        setScreen('tu-operator');
        return;
      }
      const next=r.tuResults[r.tuIndex];
      setTuMeasurementStage(r,next.stage);
    }
  }
  ...
  function setTuMeasurementStage(r,stage){
    ...
    else if(stage==='ytp'){setStage(r,'ytp');r.ytp.waiting=false}
  ...
  function renderTuRuntime(){
    ...
    document.getElementById('tu-stop').onclick=()=>{
      confirmModal('Остановить проверку по ТУ?','Проверка будет завершена как ОШИБКА СТЕНДА/прерванная проверка без нормативной таблицы результата.','Остановить',()=>{
        stopSimulation();state.tu.reportMode='stand_error';setScreen('tu-operator');
      },true)
    };
    paintTuRuntimeLive();
    startTuSimulation();
  ...
  const adminHistory = {
    '345':[
      ['14.09.2026 12:41','Производство · ClimateNormal · ЯЛК-96','НОРМА'],
      ['14.09.2026 11:20','Замена ЯЛК · YALK-1730 → YALK-1842',''],
      ['13.09.2026 16:03','Проверка по ТУ','НОРМА'],
    ],
    '346':[
      ['14.09.2026 10:12','Производство · ClimatePlus · Полная УБСИ','НОРМА'],
  ...
  function adminComponent(type,sn){
    return `<div class="component"><div class="type">${type}</div><div class="sn">${sn}</div>
      <div style="display:flex;gap:6px">
        <button class="btn btn-small" data-replace="${type}" data-sn="${sn}">Заменить</button>
        <button class="btn btn-small" data-component-history="${sn}">История</button>
      </div>
    </div>`;
  }
  ...
  function showReplaceModal(type,current){
    ...
    modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal">
      <h3>Заменить ${escapeHtml(type)}</h3>
      <p>Текущий компонент: ${escapeHtml(current)}</p>
    ...
    document.getElementById('replace-ok').onclick=()=>{
      ...
      if(owner){
        ...
        adminComponents[targetSerial][key]=sn;
        addAdminHistory(targetSerial,`Замена ${type}: ${current} → ${sn}. ${cm}`);
        modalRoot.innerHTML='';paintAdminMain();
      }
    };
  }
  function showNewUbsiModal(){
    ...
    document.getElementById('new-ok').onclick=()=>{
      ...
      adminComponents[serial]={yalk,ytp,yvp,ypp};
      adminHistory[serial]=[];
      addAdminHistory(serial,'Регистрация изделия и полного состава');
