              ...
              const StepRunResult& step, unsigned order)
{
    ...
    query.prepare(QStringLiteral(
        "INSERT INTO run_measurements(run_id,node_id,sort_order,parameter_key,title,reference,measured,lower_limit,upper_limit,unit,verdict,message) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)"));
    for (std::size_t index = 0; index < step.measurements.size(); ++index) {
        ...
        query.bindValue(11, QString::fromUtf8(value.message)); executePrepared(query); query.finish();
    }
...
RunStore::RunStore(std::string sqlitePath) : impl_(std::make_unique<Impl>())
{
    ...
    for (const auto& sql : {
             ...
             "CREATE TABLE IF NOT EXISTS run_measurements(run_id TEXT NOT NULL,node_id TEXT NOT NULL,sort_order INTEGER NOT NULL,parameter_key TEXT NOT NULL,title TEXT NOT NULL,reference REAL NOT NULL,measured REAL NOT NULL,lower_limit REAL NOT NULL,upper_limit REAL NOT NULL,unit TEXT NOT NULL,verdict TEXT NOT NULL,message TEXT NOT NULL,PRIMARY KEY(run_id,node_id,sort_order),FOREIGN KEY(run_id,node_id) REFERENCES run_steps(run_id,node_id) ON DELETE CASCADE)",
