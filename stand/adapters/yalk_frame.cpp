...
{
    std::size_t offset = 0;
    if (frame.size() == 204) offset = 4;
    if (frame.size() != offset + 200) {
        throw std::invalid_argument(
            "YALK slow frame must contain 200 bytes or a 4-byte header plus 200 bytes");
    }
    ...
    for (std::size_t w = 0; w < 100; ++w) {
        const std::uint16_t low =
            static_cast<std::uint16_t>(frame[offset + 2 * w]);

        const std::uint16_t high =
            static_cast<std::uint16_t>(frame[offset + 2 * w + 1]);
