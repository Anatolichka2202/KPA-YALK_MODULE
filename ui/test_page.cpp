...
QFrame[tuStageHeader="true"] { background:#0a1721; border:0; border-bottom:1px solid #1a3346; border-radius:0; }
QFrame[tuRow="true"] { background:#0a1722; border:1px solid #1a3346; border-radius:6px; }
QFrame[tuRowActive="true"] { background:#102c46; border:1px solid #58a5ff; border-radius:6px; }
QLabel[muted="true"] { color:#8ea6b7; }
QLabel[caption="true"] { color:#8ea6b7; font-size:11px; font-weight:700; }
QLabel[tuId="true"] { color:#67d8eb; font-size:11px; font-weight:800; }
QLabel[tuTitle="true"] { color:#9cb1c0; font-size:11px; }
...
QPushButton#skipStage { background:#352b16; border-color:#8d6a1e; color:#ffe3a1; font-weight:700; }
...
QString tuStageTitle(const QString& node, const QString& fallback)
{
    ...
    if (node == QStringLiteral("yalk_calibration"))
        return QStringLiteral("ЯЛК · калибровка по адресам 97 и 99");
    if (node == QStringLiteral("yalk_initial"))
        return QStringLiteral("ЯЛК · обрыв 80 входов · U < 0 В");
    if (node == QStringLiteral("yalk_channels"))
    ...
    if (node == QStringLiteral("yalk_cleanup"))
        return QStringLiteral("ЯЛК · снятие воздействий и контроль остатка ≤ 0,2 В");
    if (node == QStringLiteral("ytp_stream"))
        return QStringLiteral("ЯТП · запуск потока 30 каналов");
    if (node == QStringLiteral("ytp_calibration"))
        return QStringLiteral("ЯТП · калибровка по словам 32 и 31");
    if (node == QStringLiteral("ytp_channels"))
        return QStringLiteral("ЯТП · 30 каналов · точки Р4831 0 / 120 / 240 Ом");
...
struct TestPage::Impl
{
    ...
    void buildRuntime()
    {
        ...
        skipHotspot = new QPushButton(header);
        skipHotspot->setObjectName(QStringLiteral("tuSkipRevealHotspot"));
        skipHotspot->setFixedSize(24, 24);
        skipHotspot->setFlat(true);
        skipHotspot->setStyleSheet(QStringLiteral(
        ...
        headerLayout->addWidget(skipHotspot);
        headerLayout->addWidget(skipButton);
        headerLayout->addWidget(stopButton);
        layout->addWidget(header);

        runtimeContextStrip = new QFrame(runtimePageWidget);
        runtimeContextStrip->setFixedHeight(78);
        runtimeContextStrip->setProperty("panel", true);
        auto* contextLayout = new QHBoxLayout(runtimeContextStrip);
        contextLayout->setContentsMargins(16, 10, 16, 10);
        auto* contextColumn = new QVBoxLayout;
        contextColumn->setSpacing(2);
        contextCaption = caption(QStringLiteral("ТЕКУЩИЙ ЭТАП"), runtimeContextStrip);
        contextColumn->addWidget(contextCaption);
        context = new QLabel(QStringLiteral("Ожидание запуска"), runtimeContextStrip);
        ...
        auto* progressColumn = new QVBoxLayout;
        progressColumn->setSpacing(2);
        progressCaption = caption(QStringLiteral("ХОД / ДАННЫЕ"), runtimeContextStrip);
        progressCaption->setObjectName(QStringLiteral("tuRuntimeProgressCaption"));
        progressCaption->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
        progressColumn->addWidget(progressCaption);
        progressText = muted(QStringLiteral("—"), runtimeContextStrip);
        progressText->setObjectName(QStringLiteral("tuRuntimeProgress"));
        progressText->setWordWrap(true);
        progressText->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
        progressColumn->addWidget(progressText);
        contextLayout->addLayout(progressColumn, 1);
        ...
        buildStagePages();
        auto* stageColumn = new QVBoxLayout;
        stageColumn->setContentsMargins(0, 0, 0, 0);
        stageColumn->setSpacing(0);
        stageColumn->addWidget(runtimeContextStrip);
        stageColumn->addWidget(stageStack, 1);
        body->addLayout(stageColumn, 1);
        ...
        QObject::connect(skipHotspot, &QPushButton::clicked, q, [this] {
            if (!productionMode && runInProgress)
                skipButton->setVisible(!skipButton->isVisible());
        });
        QObject::connect(skipButton, &QPushButton::clicked, q, [this] {
    ...
    void buildStagePages()
    {
        ...
        powerLayout->addWidget(heading(QStringLiteral("Питание и готовность УБСИ"), 18, power));
        powerLayout->addWidget(muted(
            QStringLiteral("Проверяем готовность не более 30 с при 27 В, рабочие точки 24 / 27 / 35 В, выдержки 19 В · 5 мин и 37 В · 1 мин, общий ток ≤ 0,4 А."), power));
        ...
        initialLayout->addWidget(muted(
            QStringLiteral("Проверяем 80 входов: адреса 1–28, 32–43, 45–70, 74–87. Видны напряжение каждого входа, дискретный признак и результат; критерий обрыва U < 0 В."), initial));
        ...
        ytpLayout->addWidget(muted(
            QStringLiteral("Проверяем 30 каналов в общих точках Р4831: 0 / 120 / 240 Ом. Видны эталон Р4831, сопротивление каждого канала, min/max и результат; допуск ±0,5 % диапазона."), ytp));
    ...
    void addTuRequirement(const QString& key, const QString& id, const QString& title)
    {
        ...
        row.frame->setObjectName(QStringLiteral("tuRequirement_%1").arg(key));
        row.status->setObjectName(QStringLiteral("tuRequirementStatus_%1").arg(key));
    ...
    void buildTu()
    {
        ...
        QObject::connect(tuFlow, &TuFlowWidget::yvpBypassRequested, q, [this] {
            includeYvp->setChecked(!includeYvp->isChecked());
            tuFlow->setYvpBypassActive(!includeYvp->isChecked());
        });
    ...
    void resetRuntime()
    {
        ...
        if (productionMode) {
            ...
            showTuStageHeader({QStringLiteral("readiness")});
            const int readinessIndex = tuIndex.value(QStringLiteral("readiness"), -1);
            if (readinessIndex >= 0) {
                tuRows[readinessIndex].active = true;
                refreshTuRow(readinessIndex);
            }
        }
    }
    ...
    QStringList requirementsForNode(const QString& node, const QString& stageName = {}) const
    {
        ...
        if (node.startsWith(QStringLiteral("yvp_")) || stageName == QStringLiteral("YVP_V7_POINT"))
            return {QStringLiteral("yvp_afc"), QStringLiteral("yvp_gain")};
        return {};
    }

    void showTuStageHeader(const QStringList& keys, const QString& node = {})
    {
        if (productionMode) return;
        if (keys.isEmpty()) {
            const QString title = tuStageTitle(node, {});
            if (!title.isEmpty()) context->setText(QStringLiteral(
                "<span style='color:#eaf4fb;font-size:20px;font-weight:700'>%1</span>")
                .arg(title.toHtmlEscaped()));
            return;
        }
        const int index = tuIndex.value(keys.constFirst(), -1);
        if (index < 0 || index >= tuRows.size()) return;
            ...
            if (requirements.isEmpty()) {
                ...
                while (match.hasNext()) {
                    ...
                    for (const auto& row : tuRows)
                        if (row.id == id && !requirements.contains(row.key)) requirements << row.key;
                }
            }
    ...
    void showProcedure(Procedure procedure, const QString& node = {})
    {
        ...
        if (!productionMode) {
            ...
            if (procedure == Procedure::Finish) {
                context->setText(QStringLiteral(
                    "<span style='color:#67d8eb;font-size:14px;font-weight:800'>ИТОГ</span>"
                    "&nbsp;&nbsp;<span style='color:#eaf4fb;font-size:20px;font-weight:700'>Проверка завершена</span>"));
            }
            return;
        }
    ...
    QPushButton* skipHotspot = nullptr;
    QPushButton* skipButton = nullptr;
    QPushButton* stopButton = nullptr;
    QFrame* runtimeContextStrip = nullptr;
    QLabel* contextCaption = nullptr;
...
void TestPage::setProductionMode(bool enabled)
{
    ...
    impl_->runtimeBack->setVisible(enabled);
    impl_->sidebarStack->setFixedWidth(enabled ? 270 : 360);
    impl_->tuTitle->setVisible(enabled);
    impl_->tuSerial->setVisible(enabled);
    impl_->contextCaption->setText(enabled
        ? QStringLiteral("ТЕКУЩИЙ ЭТАП") : QStringLiteral("ТЕКУЩАЯ ПРОВЕРКА"));
    impl_->contextCaption->setVisible(true);
    impl_->progressCaption->setVisible(true);
    impl_->progressText->setVisible(true);
    impl_->runtimeContextStrip->setFixedHeight(enabled ? 78 : 88);
    impl_->runtimeContextStrip->setProperty("panel", enabled);
    impl_->runtimeContextStrip->setProperty("tuStageHeader", !enabled);
    impl_->skipHotspot->setVisible(!enabled);
    impl_->skipButton->hide();
    impl_->runtimeContextStrip->style()->unpolish(impl_->runtimeContextStrip);
    impl_->runtimeContextStrip->style()->polish(impl_->runtimeContextStrip);
...
void TestPage::updateStartAvailability()
{
    ...
    impl_->skipHotspot->setEnabled(!impl_->productionMode && impl_->runInProgress);
    impl_->skipButton->setEnabled(!impl_->productionMode && impl_->runInProgress);
    if (!impl_->runInProgress) impl_->skipButton->hide();
...
void TestPage::startSelectedTest()
{
    ...
    impl_->runTitle->setText(impl_->productionMode
        ? QStringLiteral("УБСИ %1").arg(impl_->activeSerial)
    ...
    impl_->runSubtitle->setText(impl_->productionMode
        ...
        : QStringLiteral("УБСИ SN %1").arg(impl_->activeSerial));
...
void TestPage::setRunEvent(const tu::RunEvent& event)
{
    ...
    if (stageName == QStringLiteral("MEASUREMENT")
        && (node.contains(QStringLiteral("yalk_contact"))
            || (node == QStringLiteral("yalk_channels")
                && event.data.count("signal") != 0)))
