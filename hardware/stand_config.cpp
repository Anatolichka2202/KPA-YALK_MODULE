...
StandConfig loadStandConfig(const std::filesystem::path& path)
{
    ...
    if (!supply || !yalk) {
        throw std::runtime_error("Профиль стенда должен содержать power_supply и yalk_adapter");
    }
    ...
    config.supply.outputConfirmAttempts = optional<unsigned>(
        supply, "output_confirm_attempts", 4);
    ...
    if (config.yalk.remoteHost.empty() || config.yalk.localHost.empty() || config.yalk.port == 0)
        throw std::runtime_error("Некорректная UDP-конфигурация адаптера ЯЛК");
