...
{
    explicit Impl(VisaConfig value) : config(std::move(value))
    {
#ifdef _WIN32
        open();
#else
    ...
    template <typename F> F load(const char* name)
    {
        ...
        if (!address) throw std::runtime_error(std::string("VISA function missing: ") + name);
        return reinterpret_cast<F>(address);
    }

    static void ok(ViStatus status, const char* operation)
    {
        if (status < 0) throw std::runtime_error(std::string(operation)
            + " failed, VISA status " + std::to_string(status));
    }

    void open()
    {
        if (config.resources.empty()) throw std::invalid_argument("VISA resource list is empty");
        ...
        if (!library) throw std::runtime_error("NI-VISA runtime DLL не найден в System32");
        try {
            viOpenDefaultRM = load<OpenDefaultRmFn>("viOpenDefaultRM");
            viFindRsrc = load<FindRsrcFn>("viFindRsrc");
            viOpen = load<OpenFn>("viOpen");
            viClose = load<CloseFn>("viClose");
            viSetAttribute = load<SetAttributeFn>("viSetAttribute");
            viWrite = load<WriteFn>("viWrite");
            viRead = load<ReadFn>("viRead");
            ok(viOpenDefaultRM(&resourceManager), "viOpenDefaultRM");

            char descriptor[256]{};
            for (const auto& expression : config.resources) {
                if (expression.empty()) continue;
                ViUInt32 count = 0;
                findList = 0;
                const auto status = viFindRsrc(resourceManager, expression.c_str(),
                    &findList, &count, descriptor);
                if (status == ResourceNotFound) continue;
                ok(status, "viFindRsrc");
                if (count && descriptor[0]) break;
                if (findList) { viClose(findList); findList = 0; }
                descriptor[0] = '\0';
            }
            if (!descriptor[0]) throw std::runtime_error("VISA прибор не найден");
            resource = descriptor;
            ok(viOpen(resourceManager, resource.c_str(), 0, 0, &instrument), "viOpen");
            ok(viSetAttribute(instrument, TimeoutAttribute, config.timeoutMilliseconds),
    ...
    {
        ViUInt32 written = 0;
        ok(viWrite(instrument, reinterpret_cast<const unsigned char*>(command.data()),
                   static_cast<ViUInt32>(command.size()), &written), "viWrite");
        if (written != command.size()) throw std::runtime_error("VISA записал неполную команду");
    }
    ...
    std::string query(const std::string& command, unsigned delayMilliseconds)
    {
        ...
        ViUInt32 received = 0;
        ok(viRead(instrument, buffer.data(), static_cast<ViUInt32>(buffer.size()), &received), "viRead");
        if (!received) throw std::runtime_error("VISA прибор вернул пустой ответ");
        return {reinterpret_cast<const char*>(buffer.data()), received};
    }
#endif
};

VisaInstrument::VisaInstrument(VisaConfig config)
...
#else
    (void)command; (void)delayMilliseconds;
    throw std::runtime_error("VISA доступен только на Windows");
#endif
}
