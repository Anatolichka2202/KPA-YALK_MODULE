...
uint16_t analog10bitCode(uint16_t word) {
    return (word >> 1) & 0x3FF;   // биты 1..10 → 0..1023
}

double analogCodeToVoltage(uint16_t code, double scale) {
    return static_cast<double>(code) * scale;
}

uint16_t analog9bitCode(uint16_t word) {
    return (word >> 2) & 0x1FF;   // биты 2..10 → 0..511
}

double analog9bitCodeToVoltage(uint16_t code, double scale) {
    return static_cast<double>(code) * scale;
}
