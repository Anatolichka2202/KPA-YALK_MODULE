...
int Orbita::getAnalogCount() const {
    return ctx_->getAnalogCount();
}

void Orbita::setChannelsFromPairs(const std::vector<std::pair<std::string, std::string>>& pairs) {
    ctx_->setChannelsFromPairs(pairs);
}
std::string Orbita::getAnalogChannelName(int idx) const {
    return ctx_->getAnalogChannelName(idx);
}

void Orbita::feedRawData(const int16_t* samples, size_t count) {
    ctx_->feedRawData(samples, count);
}

uint32_t Orbita::getCurrentTimeSeconds() const {
    return ctx_->getCurrentTimeSeconds();
}

int Orbita::getContactCount() const {
    return ctx_->getContactCount();
}
